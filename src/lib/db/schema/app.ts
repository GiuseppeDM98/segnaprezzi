/**
 * Application tables. Auth tables live in ./auth.ts and are
 ...
 *   metadata).
 *
 * Design note (verified empirically): price_entries.productId's FK could
 * instead be given as `onDelete: 'restrict'`, which reads more directly as
 * "blocked while entries exist". SQLite checks RESTRICT immediately per-row,
 * not deferred to end-of-statement like every other FK action (including the
 * default NO ACTION) — so a single `DELETE FROM users` that cascades to both
 * products and price_entries can fail with a spurious FK violation depending
 * on which sibling cascade SQLite happens to process first, breaking the
 * "user delete cascades everywhere" full-account-wipe guarantee this project
 * requires. Omitting onDelete (SQLite's NO ACTION default) keeps the
 * identical user-facing behavior — a direct product delete while entries
 * exist is still blocked — but checks at end-of-statement, so it no longer
 * races the user-cascade's own price_entries cleanup within the same
 * statement.
 */
...
export const products = sqliteTable(
  'products',
  {
    ...
    notes: text('notes'),
    // Why: receipt lines rarely print a package size, and the last
    // confirmed size of a product is the best guess for its next line.
...
/**
 * One imported receipt. An import record, never an observation: the price
 * observations it produces are the price_entries rows created when the user
 * confirms it.
 *
 * The file itself is deliberately NOT stored: a receipt carries
...
export const priceEntries = sqliteTable(
  'price_entries',
  {
    ...
    // Why nullable + set null: an entry outlives the import
    ...
    // Why: "2 x 1,29" on a receipt is ONE observation of the price 1,29
    // bought twice. Two rows would double-weight that month's mean; a
    // quantity keeps the mean honest while letting the inflation engine's
  ...
  (table) => [
    // Powers per-product price history and monthly bucketing.
...
/**
 * Learned receipt-line → product mappings. This table is what
